...
class Newest {
    static async handle() {
        try {
            ...
            const newestItem = result.Items.reduce((latest, item) => {
                var _a;
                // Default timestamp to a very early date if it's empty
                const timestamp = ((_a = item.data) === null || _a === void 0 ? void 0 : _a.timestamp) ? new Date(item.data.timestamp) : new Date(0);
